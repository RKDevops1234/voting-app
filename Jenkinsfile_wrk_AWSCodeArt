    ...
    stages {
        ...
        stage('Push Helm chart to AWS CodeArtifact') {
            steps {
                ...
                sh """
                        aws codeartifact put-package-origin-configuration --repository voting-app --domain petclinic --package db-1.0.0.tgz --format generic --namespace voting-app --restrictions '{"publish": "ALLOW","upstream": "BLOCK"}'
                    """
                }
            }
        }
